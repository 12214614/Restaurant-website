...
import { LogOut, Package, Clock, CheckCircle, Truck, ChefHat, XCircle, RefreshCw, CheckCircle2 } from 'lucide-react';
...
const statusOptions = [
  ...
  { value: 'ready', label: 'Ready', icon: CheckCircle2, color: 'bg-amber-500' },
...
export default function AdminDashboard({ onLogout }: AdminDashboardProps) {
  ...
  const getStatusMessage = (status: string): string => {
    const statusMessages: Record<string, string> = {
      confirmed: 'Your order has been confirmed and we\'re preparing it now!',
      preparing: 'Your order is being prepared with fresh ingredients!',
      ready: 'Great news! Your order is ready and will be dispatched shortly!',
      out_for_delivery: 'Your order is out for delivery! Our delivery partner is on the way.',
      delivered: 'Your order has been delivered! Enjoy your meal!',
      cancelled: 'Unfortunately, your order has been cancelled. Please contact us if you have questions.',
    };
    return statusMessages[status] || 'Your order status has been updated.';
  };

  const sendStatusUpdateNotifications = async (order: Order, newStatus: string) => {
    ...
    try {
      await Promise.all([
        fetch(`${supabaseUrl}/functions/v1/send-status-update-email`, {
          method: 'POST',
          headers: {
        ...
        fetch(`${supabaseUrl}/functions/v1/send-status-update-sms`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${supabaseKey}`,
  ...
  const updateOrderStatus = async (orderId: string, newStatus: string) => {
    try {
      ...
      if (currentOrder.status === newStatus) {
        alert('Order status is already set to this status.');
        return;
      }
      ...
      // Check if update was blocked by RLS or other issues
      if (error) {
        console.error('Supabase error:', error);
        console.error('Error details:', JSON.stringify(error, null, 2));
        
        // Check if it's an RLS/permission error (no rows returned means RLS blocked it)
        if (error.code === 'PGRST116' || error.message?.includes('0 rows') || error.message?.includes('Cannot coerce')) {
          const rlsError = `⚠️ Permission denied: Row Level Security (RLS) is blocking the update.\n\nPlease run the SQL fix from fix-admin-update.sql in your Supabase dashboard.\n\nSee FIX_ADMIN_UPDATE.md for instructions.`;
          alert(rlsError);
      ...
      // Handle case where update was blocked (no data returned, likely RLS)
      if (!data) {
        const rlsError = `⚠️ Update blocked: Row Level Security (RLS) policy is preventing the update.\n\nPlease run this SQL in Supabase Dashboard → SQL Editor:\n\nDROP POLICY IF EXISTS "Authenticated users can update orders" ON orders;\nCREATE POLICY "Anyone can update orders" ON orders FOR UPDATE TO anon, authenticated USING (true) WITH CHECK (true);\n\nSee fix-admin-update.sql for the full script.`;
        alert(rlsError);
        throw new Error('RLS policy blocking update - no data returned');
      }

      // Success - update with returned data
      const updatedOrder = { 
        ...currentOrder, 
        status: newStatus, 
      ...
      // Send notifications to customer (except for pending status)
      if (newStatus !== 'pending') {
        try {
          await sendStatusUpdateNotifications(updatedOrder, newStatus);
        } catch (notifError) {
      ...
      alert(`Order status updated to ${statusOptions.find(s => s.value === newStatus)?.label || newStatus}! Customer will be notified.`);
    } catch (error: any) {
      console.error('Error updating order:', error);
      const errorMessage = error?.message || error?.toString() || 'Unknown error occurred';
      
      // Don't show duplicate alerts for RLS errors
